...
df_models_avg <- read.csv("df_models_avg_01Oct2024.csv")
df_models_avg = subset(df_models_avg, select = -c(X))
df_models_avg$Binary_change <- as.factor(df_models_avg$Binary_change)

df_models_max <- read.csv("df_models_max_01Oct2024.csv")
df_models_max = subset(df_models_max, select = -c(X))
df_models_max$Binary_change <- as.factor(df_models_max$Binary_change)

df_models_avg$Hemisphere <- df_models_avg$Latitude > 0
df_models_max$Hemisphere <- df_models_max$Latitude > 0

#Trait correlations

SpeciesTraits <- df_models_avg[,c('Species','Troph', 'Length', 'Migratory')]
SpeciesTraits <- unique(SpeciesTraits)
write.table(SpeciesTraits, file="SpeciesTraits.txt")
SpeciesTraits$Migratory <- as.numeric(as.factor(SpeciesTraits$Migratory))
cor.test(SpeciesTraits$Troph, SpeciesTraits$Length, method=c("pearson"))
cor.test(SpeciesTraits$Migratory, SpeciesTraits$Length, method=c("pearson"))
cor.test(SpeciesTraits$Troph, SpeciesTraits$Migratory, method=c("pearson"))
...
###Testing for an effect of marine-freshwater migratory behaviour

model610_avg <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_a10)
summary(model610_avg)
model620_avg <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_a20)
summary(model620_avg)
model630_avg <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_a30)
summary(model630_avg)
model610_max <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_m10)
summary(model610_max)
model620_max <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_m20)
summary(model620_max)
model630_max <- glmer(Binary_change~Position_in_range*Migratory + (1|Species),family=binomial("logit"),data=df_m30)
summary(model630_max)

####testing for an effect of hemisphere

df_a10$Hemisphere <- as.factor(df_a10$Hemisphere)
model710_avg <- glmer(Binary_change~Position_in_range*Hemisphere + (1|Species),family=binomial("logit"),data=df_a10)
summary(model710_avg)

df_m10$Hemisphere <- as.factor(df_m10$Hemisphere)
model710_max <- glmer(Binary_change~Position_in_range*Hemisphere + (1|Species),family=binomial("logit"),data=df_m10)
summary(model710_max)
...
#save below as 3.5 x 3 inches portrait
...
migration.plot_avg <- plot_model(model630_avg,type = "int", terms = c("Position_in_range [all]", "Migratory"),title="")+ylab("")+xlab("") + scale_fill_manual(values=c("#0072B2","chocolate"))
migration.plot_avg <- migration.plot_avg + theme_classic() + scale_color_manual(values=c("#0072B2","chocolate"),labels=c("Not Migratory","Migratory"))
migration.plot_avg <- migration.plot_avg + theme(legend.justification = c("right", "bottom"),legend.position = c(.95,.05),legend.text = element_text(size = 12),axis.text = element_text(size=12))
migration.plot_avg <- migration.plot_avg + scale_y_continuous(limits=c(0.20,0.80),breaks = c(0.20, 0.35, 0.50, 0.65, 0.80),labels=c(0.20, 0.35, 0.50, 0.65, 0.80))
migration.plot_avg <- migration.plot_avg + effect_theme
migration.plot_avg
...
migration.plot_max <- plot_model(model610_max,type = "int", terms = c("Position_in_range [all]", "Migratory"),title="")+ylab("")+xlab("") + scale_fill_manual(values=c("#0072B2","chocolate"))
migration.plot_max <- migration.plot_max + theme_classic() + scale_color_manual(values=c("#0072B2","chocolate"),labels=c("Not Migratory","Migratory"))
migration.plot_max <- migration.plot_max + theme(legend.justification = c("right", "bottom"),legend.position = c(.95,.05),legend.text = element_text(size = 12),axis.text = element_text(size=12))
migration.plot_max <- migration.plot_max + scale_y_continuous(limits=c(0.20,0.80),breaks = c(0.20, 0.35, 0.50, 0.65, 0.80),labels=c(0.20, 0.35, 0.50, 0.65, 0.80))
migration.plot_max <- migration.plot_max + effect_theme
migration.plot_max
...
model610_slope_avg <- lmer(Slope_value~Position_in_range * Migratory + (1|Species),data=df_a10)
summary(model610_slope_avg)
model610_slope_max <- lmer(Slope_value~Position_in_range * Migratory + (1|Species),data=df_m10)
summary(model610_slope_max)
