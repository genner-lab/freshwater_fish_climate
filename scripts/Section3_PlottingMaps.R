...
order_data["29", "Order"] <- "Perciformes"
order_data["45", "Order"] <- "Cypriniformes"
order_data["208", "Order"] <- "Blenniiformes"
order_data["465", "Order"] <- "Cypriniformes"
order_data["466", "Order"] <- "Cypriniformes"

order_data <- order_data[order(order_data$Order),]
rownames(order_data) = NULL
...
TimeSeries <- ggplot(df_models_max, aes(x = Duration)) + 
  geom_histogram(bins=57) +
  theme_classic() + scale_y_continuous(limits = c(0,15000)) +
  scale_x_continuous(breaks = c(10,20,30,40,50,60,70)) +
  xlab("Duration of time series") + ylab("Number of population abundance timeseries")
TimeSeries

ggsave(TimeSeries, file="timeseries.pdf",width = 15, height = 10, unit = "cm", dpi = 300)
